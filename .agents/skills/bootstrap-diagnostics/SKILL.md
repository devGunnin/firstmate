...
- `GH_MENTIONS: <what stops or limits the GitHub mention poll>` - this home configured the GitHub mention plane and session start could not leave it fully working; [`docs/configuration.md`](../../../docs/configuration.md) ("GitHub mentions") owns the schema the line refers to.
  A configuration problem means the plane is stopped and the shim retired, so tagged comments in watched repositories are reaching nobody until the named field is corrected; never guess a default for it, because the trusted-login list decides who firstmate obeys.
  A `nothing to watch` line means the plane is armed and correct but resolves to no repository, so report it as a registration gap rather than a failure: register the project here, or add the repository to the config's `repos` list.
  A line naming one registered project means only that project contributes nothing, most often because it has no clone in this home or its origin is not on github.com; the rest of the watched set is unaffected.
  A plane the captain paused with `enabled` set to false emits no line at all, so this diagnostic never means "the captain turned it off": every `GH_MENTIONS:` line is something that needs looking at.
...
- `WATCH_CADENCE: <what the cadence is now, and which plane asked for it>` - `config/x-mode.env` is this home's ONE watcher cadence, carrying the fastest interval any enabled plane asked for (`docs/configuration.md` "Watcher cadence"), so this line is about that shared file and NOT about Relay; never route it to Relay handling because it does not carry the `FMX:` prefix.
  A `could not settle` line means the named plane asked for a faster sweep than the watcher's default and bootstrap could not write the file, so that plane is polled at the default interval until the underlying `config/` write problem is fixed - inspect directory permissions and whether the path was replaced by a symlink, then rerun bootstrap.
  A `no enabled plane asks` line is the normal wind-down after the last fast-cadence plane was turned off; apply it to a running watcher the same way an `FMX:` cadence transition is applied, through the emitted harness supervision protocol.
  An `asked for a <n>s sweep` line is the opposite transition - a plane opted in, or its configured interval changed - and it is the one case where the printed repair pointer matters most: until the home-scoped watcher is restarted through the emitted harness supervision protocol it keeps sweeping at its old interval, so the plane that asked for the faster sweep is still polled at the old one.
  An unchanged interval prints nothing, so any of these lines means the cadence genuinely moved this session.
